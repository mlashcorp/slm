...
def get_lr_multiplier(step: int, warmup_steps: int, num_iterations: int, warmdown_ratio: float, final_lr_frac: float) -> float:
    """Compute learning rate multiplier (warmup-constant-warmdown schedule)."""
...
def get_momentum(step: int, num_iterations: int, warmdown_ratio: float) -> float:
    ...
    Warmup to 0.95 over 300 steps, warmdown to 0.85 during LR warmdown.
    """
    warmdown_steps = round(warmdown_ratio * num_iterations)
...
def train(args):
    ...
    if args.depth == 12:
        ...
        config = NanoChatConfig(n_layer=args.depth)
    ...
    print(f"  Total (actual): {model.num_parameters():,} parameters")
    ...
    if args.num_iterations > 0:
        ...
        tokens_per_step = args.batch_size * config.block_size
    ...
    total_tokens = num_iterations * args.batch_size * config.block_size
    scaling_params = param_counts["transformer_matrices"] + param_counts["lm_head"]

    print(f"\nTraining configuration:")
    print(f"  Total iterations: {num_iterations:,}")
    print(f"  Batch size: {args.batch_size}")
    print(f"  Sequence length (block_size): {config.block_size}")
    print(f"  Tokens per step: {args.batch_size * config.block_size:,}")
    ...
    while step < num_iterations:
        ...
        tokens_per_sec = args.batch_size * config.block_size / dt
        ...
        if args.save_every > 0 and step % args.save_every == 0 and step > 0:
            checkpoint_dir = Path(args.checkpoint_dir) / f"d{config.n_layer}"
    ...
    print(f"  Total tokens: {step * args.batch_size * config.block_size:,}")
    print(f"  Total time: {total_time/60:.1f} minutes")
    print(f"  Average tokens/sec: {step * args.batch_size * config.block_size / total_time:.0f}")
...
def main():
    ...
    # Model
    parser.add_argument("--depth", type=int, default=24, help="Model depth (n_layer: 12, 24, 26)")
    parser.add_argument("--max-seq-len", type=int, default=2048, help="Sequence length (block_size)")
